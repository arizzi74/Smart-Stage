...
| §4: Windows native renderer, explicit endpoint, one timeline and compiled-in COM bridge | `bridge_windows.cpp`; documented Media Session/EVR fallback and common MTA; real format inspection and silent-video playback on both Windows runners | Windows runners expose no audio endpoints, so actual audio-renderer playback and routing remain unverified |
...
| §12: schema, per-user storage, atomic replacement, backup, corruption, instance lock, silent restart | Store tests run on all four target OS jobs; corrupt originals retained; native app restart restores labels/order and remains stopped/stage-disabled | Sudden power-loss/storage failure and clean-user permission scenarios |
