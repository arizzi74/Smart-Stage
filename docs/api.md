...
| `POST /api/admin-presence` | `{}` marks an authenticated Admin page as present for ten seconds; HTTP 200 `{present:true,capabilities:{chooseFiles:true/false}}`. An active Admin SSE also counts; its last disconnect clears the old lease. Ordinary session/state probes do not suppress browser launch. |
