...
[Executables, Mac apps and checksums](https://github.com/arizzi74/Smart-Stage/releases/tag/v0.1.0-preview.5)
...
download the matching `.app.zip`, extract `Smart Stage.app` and move it to
Applications if desired. Opening it starts Smart Stage in Terminal, where the
pairing keys and Ctrl+C remain available. The curl command installs the CLI.
...
`VERSION=v0.1.0-preview.5` sets metadata. Output:
